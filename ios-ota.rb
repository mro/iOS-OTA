...
class IosIpa
  ...
  def self.is_ios_device
    !( /\s\((iPhone|iPod|iPad);\s/.match( ENV['HTTP_USER_AGENT'] ).nil? )
  end
  ...
  def self.build_ota( ipa_path, ipa_url )#:doc:
    ...
    begin
      if ! FileTest.exist?('iTunesArtwork')
        ...
        File.symlink('../../../iTunesArtwork', 'iTunesArtwork') unless $? == 0
      end
      art = nil
      IO.popen("file --dereference --brief --mime-type iTunesArtwork") { |f| art = f.read }
   ...
   if ! FileTest.exist?('Icon.png')
     ...
     File.symlink('iTunesArtwork.png', 'Icon.png') unless $? == 0
   end
