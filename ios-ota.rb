...
class IphoneOta
	...
	def self.most_recent base_url, ipa_files=File.join('deploy','v*', 'Debug', '*.ipa')
	...
	def self.each_version base_url, ipa_files=File.join('deploy','v*', 'Debug', '*.ipa'), sort_ascending=false
		...
		a = ipa_files.collect{|ipa| IphoneOta.new create_manifest_from_ipa(ipa, base_url, false)}
		if sort_ascending
			a.sort!{ |x,y| x <=> y }
		else
			a.sort!{ |x,y| y <=> x }
	...
	def ota_url
		m_url = ipa_url.to_s.gsub(/\/[^\/]+\.ipa$/, '/manifest.plist')
